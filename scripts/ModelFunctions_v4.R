...
############## and updated according to Aldrin et al. (2022: manuscript) 
...
extractCoef <- function(x,
  ...
  ## If posterior.sample = "mean", extract mean across posterior samples
  if(posterior.sample == "mean") {out <- round(rowMeans(x), ndigits)} else {
      ## If not, extract values for posterior sample no. 'posterior.sample' 
     out <- round(x[, posterior.sample], ndigits)}
...
set.Coef.fix <- function(...){
  Coef.fix <- c(
    delta.dur.DMcht = 7,  # Duration constant (days) Deltamethrin
    delta.dur.EMcht = 60, # Duration constant (days) Emamectin
...
extractRanef <- function(paramSamples, ...){ 
  ...
  uStar.TEFcht.all <- paramSamples[grep("uStar.TEFcht.", row.names(paramSamples)),] # New
  uStar.IMcht.all <- paramSamples[grep("uStar.IMcht.", row.names(paramSamples)),] # New
  ...
  uStar.TEFcht.samples <- Samples.list(uStar.TEFcht.all)
  uStar.IMcht.samples <- Samples.list(uStar.IMcht.all)
...
return(
  list(
    ...
    uStar.TEFcht.all = uStar.TEFcht.all,
    uStar.IMcht.all = uStar.IMcht.all,
    ...
    uStar.TEFcht.samples = uStar.TEFcht.samples,
    uStar.IMcht.samples = uStar.IMcht.samples,
...
trt.sample.postsamp <- function(trt, Ncages, b = "random", ...){ # Draw from posterior samples
  ...
  # The argument 'trt' is the type of chemical or physical treatment
    ...
    if(trt=="TEFcht") {uStar <- RE.all$uStar.TEFcht.all; uStar.samples <- RE.all$uStar.TEFcht.samples}
    if(trt=="IMcht") {uStar <- RE.all$uStar.IMcht.all; uStar.samples <- RE.all$uStar.IMcht.samples}
  ...
  if(b=="random") B <- sample(1:(dim(uStar)[2]), 1) else B <- b 
...
trt.sample.rangen <- function(trt, Ncages, ...){ # Randomly generate cage-specific effects for each stage
    ...
    if(trt=="DBcht") {out <- array(rnorm(n=Ncages*3, mean=Coef["lambda.DBcht"],sd=Coef["sigma.EMcht.2"]^.5),
                                   dim=c(Ncages,3),dimnames=list(1:Ncages,c("CH","PA","Af"))) }
    if(trt=="TEFcht") {out <- array(rnorm(n=Ncages*3, mean=Coef["lambda.TEFcht"],sd=Coef["sigma.EMcht.2"]^.5),
                                   dim=c(Ncages,3),dimnames=list(1:Ncages,c("CH","PA","Af"))) }
    if(trt=="IMcht") {out <- array(
       rnorm(n=Ncages*3,
         mean=rep(c(Coef["lambda.IMScht"],Coef["lambda.IMcht"],Coef["lambda.IMcht"]),each=Ncages),
         sd=Coef["sigma.DMcht.2"]^.5),
       dim=c(Ncages,3),dimnames=list(1:Ncages,c("CH","PA","Af"))) }
...
# Function to prepare environmental variables for the analysis
prepare.EnvData <- function(EnvListElement, Ndays, ...){
  ...
  ST <- wk.interpolate(as.numeric(EnvListElement$ST))[1:Ndays]
  N.AF.Ext <- wk.interpolate(as.numeric(EnvListElement$N.AF.Ext))[1:Ndays]
  A.Ext <- wk.interpolate(as.numeric(EnvListElement$A.Ext))[1:Ndays]
  ST.init <- wk.interpolate(as.numeric(EnvListElement$ST.init))
  N.AF.Ext.init <- wk.interpolate(as.numeric(EnvListElement$N.AF.Ext.init))
  A.Ext.init <- wk.interpolate(as.numeric(EnvListElement$A.Ext.init))
  Lusegrense <- as.numeric(rep(EnvListElement$Lusegrense, each=7))[1:Ndays]
  ...
  AF.Obs <- wk.interpolate2(as.numeric(EnvListElement$AF.obs))[1:Ndays]
  ...
  Treat.Obs <- wk.interpolate3(as.numeric(EnvListElement$Treat))[1:Ndays]
  ...
  attr(out, "start.mo") <- as.numeric(EnvListElement$Month)
  attr(out, "start.yr") <- as.numeric(EnvListElement$Year)
  attr(out, "PO") <- as.numeric(EnvListElement$PO)
  attr(out, "FarmID") <- as.numeric(EnvListElement$FarmID)
  attr(out, "Lon_UTM") <- as.numeric(EnvListElement$Lon_UTM)
  attr(out, "Lat_UTM") <- as.numeric(EnvListElement$Lat_UTM)
  
  return(out)
}

# Function to find and prepare environmental data (temperature, external lice pressure) 
...
find.EnvData <- function(EnvList, Region, start.mo, Ndays, farmID = NULL, start.yr = NULL, ...){
  
  if(is.null(farmID) | is.null(start.yr)){
    ...
    #  Nmonths >= Ndays/30) # We now have at least 600 d env.data
    if(length(env.alts)==0) stop("Not enough environmental data for this area, period and production length")
  }
  
  if(!is.null(farmID) & !is.null(start.yr)){
    FarmIDs <- unlist(lapply(EnvList, function(x) x$FarmID))
    Years <- unlist(lapply(EnvList, function(x) x$Year))
    env.alts <- which(FarmIDs == farmID &
                        Years == start.yr)
    if(length(env.alts)==0) stop("No environmental data are found for this location and start year")
  }
  ...
  out <- prepare.EnvData(EnvData.select, Ndays, ...)
...
define.model.variables <- function(Ncages, Ndays,...){
  ...
  ux.DBcht.AF <- array(0, dim = c(Ndays, a.max.AF, Ncages))  # Diflubenzuron 
  ux.TEFcht.PA <- array(0, dim = c(Ndays, a.max.PA, Ncages))  # Teflubenzuron 
  ux.TEFcht.AF <- array(0, dim = c(Ndays, a.max.AF, Ncages))  # Teflubenzuron 
  ux.IMcht.CH <- array(0, dim = c(Ndays, a.max.CH, Ncages))  # Imidacloprid
  ux.IMcht.PA <- array(0, dim = c(Ndays, a.max.PA, Ncages))  # Imidacloprid
  ux.IMcht.AF <- array(0, dim = c(Ndays, a.max.AF, Ncages))  # Imidacloprid
  ...
  use.TEFcht <- array(0, dim=c(Ndays, Ncages))
  use.IMcht <- array(0, dim=c(Ndays, Ncages))
  ...
  return(
    list(
      ...
      ux.DBcht.AF = ux.DBcht.AF, 
      ux.TEFcht.PA = ux.TEFcht.PA, 
      ux.TEFcht.AF = ux.TEFcht.AF, 
      ux.IMcht.CH = ux.IMcht.CH,
      ux.IMcht.PA = ux.IMcht.PA, 
      ux.IMcht.AF = ux.IMcht.AF,
...
model.initialize <- function(SV, RE, Coef, Ncages, Ndays, ...){
  ...
  m.nat.CO[,] <- invlogit(Coef["lambda0.RCOnat"] + Coef["lambda1.RCOsal"] * x.sal)  # Eq. 4-6, Aldrin & Huseby 2019. 
# v3: m.nat.CO[,] <- invlogit(Coef["lambda0.RCOnat"])  # Eq. 4-6, Aldrin & Huseby 2019. NB! No salinity correction here.
...
# Jan. 2022: delta.Rp er brukt i denne likningen, men ikke for rekrutteringsrate
  d.m.R <- med.time(SV$ST.stageav[,1:a.max.R], Coef["delta.Em10"] + Coef["delta.Nm10"], Coef["delta.Rp"])
  # v3: d.m.R <- med.time(SV$ST.stageav[,1:a.max.R], Coef["delta.Em10"] + Coef["delta.Nm10"], Coef["delta.CHp"])
...
model.spinup <- function(SV, RE, Coef, Ncages, ...){
  ...
  # (assumes random effect as at time step 1)
  N.R.init[,1] <- r.Ext.init * SV$N.AF.Ext.init * RE$e.Ext.f[1]
  ## Note: this code was corrected 7.3.2022 (the 2 last factors forgotten)
...
update_m.clf.PA <- function(SV, t, Ncages,...){
  ...
  if(Ncages ==1) N.lice.cages <- sum(SV$N.PA[t, , ])
  if(Ncages >1) N.lice.cages <- colSums(SV$N.PA[t, , ])
...
update_m.clf.AF <- function(SV, t, Ncages,...){
  ...
  # if(Ncages ==1) N.lice.cages <- sum(SV$N.PA[t, , ]) + sum(SV$N.AF[t, , ])
  # if(Ncages >1) N.lice.cages <- colSums(SV$N.PA[t, , ]) + colSums(SV$N.AF[t, , ])
  if(Ncages ==1) N.lice.cages <- sum(SV$N.AF[t, , ])
  if(Ncages >1) N.lice.cages <- colSums(SV$N.AF[t, , ])
  which.lice <- which(N.lice.cages > 0)
  m.lump.cages <- rep(0, Ncages)
  m.lump.cages[which.lice] <- (1 - exp(-Coef["gamma.lump"] * 
                                 N.lice.cages[which.lice] / SV$N.SAL[t, which.lice])) * 
...
# Hydrogen peroxide
update_ux.HPcht <- function(SV, t, Ncages, Ndays,...){
  ...
  if(sum(SV$use.HPcht[t, ]) > 0){ # If treatment was applied in at least one cage at time t
    ...
    for(cage.j in 1:Ncages){             # Loop on cages
      if(SV$use.HPcht[t, cage.j]==1) {      # If cage.j was treated
            out$ux.HPcht.PA[t, , cage.j] <- HPcht.eff[cage.j, "PA"] # Effect on PA
            out$ux.HPcht.AF[t, , cage.j] <- HPcht.eff[cage.j, "Af"]  # Effect on A
      }
    }
  }
  return(out)
...
# Deltamethrin
update_ux.DMcht <- function(SV, t, Ncages, Ndays, a.max.CH = 60, a.max.PA = 60, a.max.AF = 80,...){
  out <- SV[c("ux.DMcht.CH", "ux.DMcht.PA", "ux.DMcht.AF")]
    ...
    for(cage.j in 1:Ncages){         # Loop on cages
      if(SV$use.DMcht[t, cage.j]==1) {     # If cage.j was treated
        t1 <- t       # Start time of effect     
        # End time of effect:
        t2 <- min(t1 + Coef["delta.dur.DMcht"] - 1, Ndays)  
        for(t.eff in t1:t2){
          a.min <- t.eff - t + 1        # Minimum age-index (= age + 1) of effect (Eq. 19)
          if(a.min <= a.max.CH){
            out$ux.DMcht.CH[t.eff, a.min:a.max.CH, cage.j] <- DMcht.eff[cage.j, "CH"]/(t2-t1+1) # Effect on CH
          }
          a.min <- max(t.eff - t - round(SV$d.m.CH[t, 1]) + 1, 1)
          if(a.min <= a.max.PA){
            out$ux.DMcht.PA[t.eff, a.min:a.max.PA, cage.j] <- DMcht.eff[cage.j, "PA"]/(t2-t1+1) # Effect on PA
          }
          a.min <- max(t.eff - t - round(SV$d.m.CH[t, 1] + SV$d.m.PA[t, 1]) + 1, 1)
          if(a.min <= a.max.AF){
            out$ux.DMcht.AF[t.eff, a.min:a.max.AF, cage.j] <- DMcht.eff[cage.j, "Af"]/(t2-t1+1)  # Effect on A
          } 
        }
      }
    }
...
# Azamethiphos
update_ux.AZcht <- function(SV, t, Ncages, Ndays,...){
  out <- SV[c("ux.AZcht.PA", "ux.AZcht.AF")]
    ...
    for(cage.j in 1:Ncages){         # Loop on cages
      out$ux.AZcht.PA[t, , cage.j] <- AZcht.eff[cage.j, "PA"] # Effect on PA
      out$ux.AZcht.AF[t, , cage.j] <- AZcht.eff[cage.j, "Af"]  # Effect on A
    }
  }
  return(out)
}
...
# Emamectin
update_ux.EMcht <- function(SV, t, Ncages, Ndays, d=14, ...){
  # New 4.1.2022: parameter d is the number of days the feed treatment is applied
  out <- SV[c("ux.EMcht.CH", "ux.EMcht.PA", "ux.EMcht.AF")]
    ...
    for(cage.j in 1:Ncages){         # Loop on cages
      if(SV$use.EMcht[t, cage.j]==1) {     # If cage.j was treated
        t1 <- t       # Start time of effect     
        # End time of effect :
        t2 <- min(t1 + Coef["delta.dur.EMcht"] + d - 1, Ndays)  
        for(t.eff in t1:t2){
            out$ux.EMcht.CH[t.eff, , cage.j] <- EMcht.eff[cage.j, "CH"]/Coef["delta.dur.EMcht"] # Effect on CH
            out$ux.EMcht.PA[t.eff, , cage.j] <- EMcht.eff[cage.j, "PA"]/Coef["delta.dur.EMcht"] # Effect on PA
            out$ux.EMcht.AF[t.eff, , cage.j] <- EMcht.eff[cage.j, "Af"]/Coef["delta.dur.EMcht"]  # Effect on A
        }
      }
    }
  }
...
# Diflubenzuron
update_ux.DBcht <- function(SV, t, Ncages, Ndays, d=14, ...){
  # New 4.1.2022: parameter d is the number of days the feed treatment is applied
  out <- SV[c("ux.DBcht.PA", "ux.DBcht.AF")]
    ...
    for(cage.j in 1:Ncages){         # Loop on cages
      if(SV$use.DBcht[t, cage.j]==1) {     # If cage.j was treated
        t1 <- t       # Start time of effect     
        # End time of effect on molting into PA:
        t2 <- min(t1 + d + round(SV$d.m.CH[t, 1]) - 1, Ndays)  
        # End time of effect on molting into AF:
        t3 <- min(t1 + d + round(SV$d.m.CH[t, 1] + SV$d.m.PA[t, 1]) - 1, Ndays)  
        out$ux.DBcht.PA[t1:t2, 1, cage.j] <- DBcht.eff[cage.j, "PA"] # Effect on CH->PA
        out$ux.DBcht.AF[t1:t3, 1, cage.j] <- DBcht.eff[cage.j, "Af"] # Effect on PA->AF
      }
    }
  }
  return(out)
...
# Teflubenzuron
update_ux.TEFcht <- function(SV, t, Ncages, Ndays, d=14,...){
  # New 4.1.2022: parameter d is the number of days the feed treatment is applied
  out <- SV[c("ux.TEFcht.PA", "ux.TEFcht.AF")]
  if(sum(SV$use.TEFcht[t, ]) > 0){ # If treatment was applied in at least one cage at time t
    TEFcht.eff <- trt.sample("TEFcht", Ncages) # Draw a posterior sample with cage-specific effects for each stage
    TEFcht.eff <- log(1 + exp(TEFcht.eff)) # Transformed coefficients (Eq. 22, Aldrin et al. 2017)
    for(cage.j in 1:Ncages){         # Loop on cages
      if(SV$use.TEFcht[t, cage.j]==1) {     # If cage.j was treated
        t1 <- t       # Start time of effect     
        # End time of effect on molting into PA:
        t2 <- min(t1 + d + round(SV$d.m.CH[t, 1]) - 1, Ndays)  
        # End time of effect on molting into AF:
        t3 <- min(t1 + d + round(SV$d.m.CH[t, 1] + SV$d.m.PA[t, 1]) - 1, Ndays)  
        out$ux.TEFcht.PA[t1:t2, 1, cage.j] <- TEFcht.eff[cage.j, "PA"] # Effect on CH->PA
        out$ux.TEFcht.AF[t1:t3, 1, cage.j] <- TEFcht.eff[cage.j, "Af"] # Effect on PA->AF
      }
    }
  }
  return(out)
...
# Imidacloprid
update_ux.IMcht <- function(SV, t, Ncages, Ndays,...){
  out <- SV[c("ux.IMcht.CH", "ux.IMcht.PA", "ux.IMcht.AF")]
  if(sum(SV$use.IMcht[t, ]) > 0){ # If treatment was applied in at least one cage at time t
    IMcht.eff <- trt.sample("IMcht", Ncages) # Draw a posterior sample with cage-specific effects for each stage
    IMcht.eff <- log(1 + exp(IMcht.eff)) # Transformed coefficients (Eq. 22, Aldrin et al. 2017)
    for(cage.j in 1:Ncages){         # Loop on cages
      if(SV$use.IMcht[t, cage.j]==1) {     # If cage.j was treated
            out$ux.IMcht.CH[t, , cage.j] <- IMcht.eff[cage.j, "CH"] # Effect on CH
            out$ux.IMcht.PA[t, , cage.j] <- IMcht.eff[cage.j, "PA"] # Effect on PA
            out$ux.IMcht.AF[t, , cage.j] <- IMcht.eff[cage.j, "Af"] # Effect on A
      }
    }
  }
  return(out)
...
update_d.CO <- function(SV, RE, t, Ndays, a.max.CO = 60,...){
  ...
  if(t < Ndays){
    # Cage-specific expected rate at logit scale (Eq. 28, Aldrin et al. 2017), Eq. 14 Aldrin & Huseby 2019
     # Reformulated to Eq. 31 Aldrin 2022 Oct. 2022
    if(sum(SV$N.SAL[t+1,]) > 0){
      d.CO.t <- (SV$N.SAL[t+1,]/sum(SV$N.SAL[t+1,])) *
        invlogit(RE$delta0.CO.f) *
        invlogit(Coef["delta1.CO"] * (log(SV$W.SAL[t+1,]) - 0.55)) *
        invlogit(Coef["delta2.CO"] * (log(SV$ST[t+1]) - log(9)) + Coef["delta3.CO"] * (log(SV$ST[t+1]) - log(9))^2) *
        invlogit(Coef["delta4.CO"] * log(sum(SV$N.SAL[t+1,])))
    } else {
      d.CO.t <- 0
    }
    ...
    # # We assume that development may only happen for a >= 1:
    # out[,1,] <- 0
    # 
    } else out[,,] <- 0
  return(out)  
...
create.SV_RE_start <- function(EnvList_local, model.settings){
  ...
  ## Set model settings
  # If farmID and start year are given, this is used:
  if(!is.null(model.settings$farmID) & !is.null(model.settings$start.yr)){
    farmID <- model.settings$farmID
    start.yr <- model.settings$start.yr
  }
  # If not, a random farm in a given PO or region and start month is chosen:
  if(is.null(model.settings$farmID) | is.null(model.settings$start.yr)){
  ...
  if(!is.null(model.settings$farmID) & !is.null(model.settings$start.yr)){
    ED <- find.EnvData(EnvList, farmID = farmID, start.yr = start.yr, Ndays = Ndays) 
  }
  if(is.null(model.settings$farmID) | is.null(model.settings$start.yr)){
    ED <- find.EnvData(EnvList, Region = Region, start.mo = start.mo, Ndays = Ndays) 
  }
  ...
  return(list(SV=SV, RE=RE, Coordinates=c(attr(ED,"Lon_UTM"), attr(ED,"Lat_UTM"))))
}
...
update.licecount1 <- function(SV_local, RE_local, t_local, model.settings,...){
  ...
  # Do not count fish if there is no fish in cage
  SV$n.SAL[t,][SV$N.SAL[t,]==0] <- 0
...
update.licecontrol <- function(SV_local, t_local, model.settings,...){
  ...
  if(t <= (Ndays - treat.delay)) {
    if(do_treat == 1){
      if(is.numeric(which_treat)){index <- which_treat} else {index <- 1:Ncages}
  ...
  if(do_addclf == 1){
    if(is.numeric(which_clf)){index <- which_clf} else {index <- 1:Ncages}
    if(t==1 | t==Ndays) t.add <- t else t.add <- t+1
    SV$S.wrasse[t.add,] <- SV$N.SAL[t, index] * clfratio
  }
...
update.licedynamics <- function(SV_local, RE_local, t_local, model.settings,...){
  ...
  for(t in t_start:t_stop){
    ...
    # chemical treatments
    # (hydrogen peroxide, deltamethrin (pyrethroid), azamethiphos, diflubenzuron, teflubenzuron, imidacloprid)
    SV[c("ux.HPcht.PA", "ux.HPcht.AF")] <- update_ux.HPcht(SV, t, Ncages, Ndays)
    SV[c("ux.DMcht.CH", "ux.DMcht.PA", "ux.DMcht.AF")] <- update_ux.DMcht(SV, t, Ncages, Ndays)
    SV[c("ux.AZcht.PA", "ux.AZcht.AF")] <- update_ux.AZcht(SV, t, Ncages, Ndays)
    SV[c("ux.EMcht.CH", "ux.EMcht.PA", "ux.EMcht.AF")] <- update_ux.EMcht(SV, t, Ncages, Ndays)
    SV[c("ux.DBcht.PA", "ux.DBcht.AF")] <- update_ux.DBcht(SV, t, Ncages, Ndays)
    SV[c("ux.TEFcht.PA", "ux.TEFcht.AF")] <- update_ux.TEFcht(SV, t, Ncages, Ndays)
    SV[c("ux.IMcht.CH", "ux.IMcht.PA", "ux.IMcht.AF")] <- update_ux.IMcht(SV, t, Ncages, Ndays)
  ...
  return(list(SV=SV, t_stop=t_stop))
}

update_SV <- function(SV_local, RE_local, t_local, model.settings_local){
  ...
  # Relevant model settings  
  Ncages <- model.settings_local$Ncages
  ncount <- model.settings_local$ncount
  n2count <- model.settings_local$n2count

  # Apply lice control measures
  if(model.settings_local$do_treat==1 | model.settings_local$do_addclf==1){

        SV_updated <- update.licecontrol(SV_local = SV, t_local = t_start, 
                                     model.settings = model.settings_local)
    
    SV <- SV_updated
  }

  # Update dynamics until next count:
  # (for t=1, start simulation at t, otherwise at t+1)
  if(t_start==1) t_startsim <- t_start else t_startsim <- t_start + 1
  SV_t_updated <- update.licedynamics(SV_local = SV, RE_local = RE, t_local = t_startsim, 
                                      model.settings = model.settings_local)
  
  # Extract SV and time from SV_t_updated: 
  SV <- SV_t_updated$SV
  t_stop <- SV_t_updated$t_stop

  # Count lice # 1.3.22: do not use wrapper functions update.licecount1 and 2
  t <- t_stop
  ...
  # SV_updated <- update.licecount1(SV_local = SV, RE_local = RE, t_local = t_stop, 
  #                                 model.settings = model.settings_local)
  # 
  # SV <- SV_updated
  
  # A possible second count of lice
  # SV_updated <- update.licecount2(SV_local = SV, RE_local = RE, t_local = t_stop, 
  #                                 model.settings = model.settings_local)
  # SV <- SV_updated
