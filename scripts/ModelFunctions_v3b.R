...
# Hydrogen peroxide
update_ux.HPcht <- function(SV, t, Ncages, Ndays, a.max.PA = 60, a.max.AF = 80,...){
...
# Azamethiphos
update_ux.AZcht <- function(SV, t, Ncages, Ndays, a.max.PA = 60, a.max.AF = 80,...){
  out <- SV[c("ux.AZcht.PA", "ux.AZcht.AF")]
    ...
    for(cage.j in 1:Ncages){         # Loop on cages
      ...
      # 2.3.22: Note that CM is modelled as DM, with no "CM" variable, because
      # we assume that the effects of deltamethrin and cypermethrin are equal.
      use.DMCMcht.t <- F # Indicator variable for DM or CM use
      if(SV$use.DMcht[t, cage.j]!=0){use.DMCMcht.t <- T} # Used the same day
      if(t>1){if(SV$use.DMcht[t-1, cage.j]!=0) use.DMCMcht.t <- T} # Used the day before
      if(SV$ux.DMcht.AF[t, a.max.AF, cage.j] != 0){use.DMCMcht.t <- T}   # Used previously, but still in effect
      if(SV$use.AZcht[t, cage.j]==1 & use.DMCMcht.t==F) {        # If cage.j was treated and DM or CM not used
...
# Emamectin
update_ux.EMcht <- function(SV, t, Ncages, Ndays, a.max.CH = 60, a.max.PA = 60, a.max.AF = 80,...){
...
# Diflubenzuron
update_ux.DBcht <- function(SV, t, Ncages, Ndays, a.max.CH = 60, a.max.PA = 60,...){
...
update.licedynamics <- function(SV_local, RE_local, t_local, model.settings,...){
  ...
  for(t in t_start:t_stop){
    ...
    SV[c("ux.DBcht.CH", "ux.DBcht.PA")] <- update_ux.DBcht(SV, t, Ncages, Ndays)
